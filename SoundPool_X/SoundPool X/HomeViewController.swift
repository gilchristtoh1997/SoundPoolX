...
class HomeViewController: UITableViewController, UITabBarControllerDelegate{
    ...
    func touchingToolbar()
    {
        UIView.animate(withDuration: 0.5) {
            if let screenWindow = UIApplication.shared.keyWindow
            {
                ...
                BarPauseButton = UIButton(frame: CGRect(x: scrollView.frame.size.width - 200, y: (scrollView.frame.size.height) - 230, width: 50, height: 70))
                ...
                forwardButton.backgroundColor = UIColor.white
                forwardButton.setImage(UIImage(named: "forward"), for: .normal)
                forwardButton.addTarget(self, action: #selector(self.newViewforwardAction(_:)), for: .touchUpInside)
                
                
                
                self.songLabel.frame = CGRect(x: 0, y: (scrollView.frame.size.height) - 375, width: scrollView.frame.size.width, height: 70)
                self.songLabel.textAlignment = NSTextAlignment.center
                self.songLabel.text = SongLabel.text
                ...
                backButton.addTarget(self, action: #selector(self.newViewBackAction(_:)), for: .touchUpInside)
                
                scrollView.addSubview(backButton)
                scrollView.addSubview(BarPauseButton)
                scrollView.addSubview(forwardButton)
                scrollView.addSubview(self.songLabel)
    ...
    func newViewBackAction(_ sender: UIButton!) {
        ...
        if(list.contains(path))
        {
            ...
            if(count == 0)
            {
                print("if in newViewbackaction")
                currentSong = list[count]
                SongLabel.text = currentSong
                songLabel.text = SongLabel.text
                songImageView.image = UIImage(named: songLabel.text!)
                print(currentSong)
                ...
                if(BarPauseButton.currentImage?.isEqual(UIImage(named: "play")))!
                {
                    AudioPlayer.pause()
                    playControl = paused
                }
                else if(BarPauseButton.currentImage?.isEqual(UIImage(named: "pause")))!
                {
                    AudioPlayer.play()
                    playControl = playing
                }
            ...
            {
                print("else in newViewbackaction")
    ...
    func newViewforwardAction(_ sender: UIButton!) {
        songsPlayed = songsPlayed + 1
        let path: String = (((AudioPlayer.currentItem?.asset) as? AVURLAsset)?.url.absoluteString)!.replacingOccurrences(of: "http://soundpool.cs.loyola.edu/Song_Folder/a_songs/", with: "").replacingOccurrences(of: "%20", with: " ").replacingOccurrences(of: ".mp3", with: "")
        var count = 0
        ...
        if(list.contains(path))
        {
            ...
            {
                print(" if in newViewforwardaction")
                currentSong = list[count]
                SongLabel.text = currentSong
                songLabel.text = SongLabel.text
                songImageView.image = UIImage(named: songLabel.text!)
                ...
                let Item = AVPlayerItem(url: currentSongUrl!)
                BarPauseButton.setImage(UIImage(named: "play"), for: .normal)
                AudioPlayer = AVPlayer(playerItem:Item)
                AudioPlayer.pause()
                playControl = paused
            ...
            {
                print("else in newViewforwardaction")
                currentSong = list[count + 1]
                SongLabel.text = currentSong
                songLabel.text = SongLabel.text
                songImageView.image = UIImage(named: songLabel.text!)
        ...
        if velocity.y > 30 {
            UIView.animate(withDuration: 0.5){
                if let keyWindow =  UIApplication.shared.keyWindow
                {
                    ...
                    toolBarPauseButton.setImage(BarPauseButton.currentImage, for: .normal)
                }
            }
        }
    ...
    func updateSongsPlays(song: String)
    {
        ...
        Database.database().reference().child("Songs").observeSingleEvent(of: .value, with: { (snapshot) in
            if let dictionary = snapshot.value as? [String: AnyObject]
            {
                ...
                let newSong = song.replacingOccurrences(of: ".", with: "").replacingOccurrences(of: "%20", with: " ")
                print(newSong)
    ...
    func backAction(_ sender: UIButton!) {
        ...
        if(list.contains(path))
        {
            ...
            if(count == 0)
            {
                ...
                let Item = AVPlayerItem(url: currentSongUrl!)
                AudioPlayer = AVPlayer(playerItem:Item)
                if(toolBarPauseButton.currentImage?.isEqual(UIImage(named: "pause")))!
                {
                    AudioPlayer.pause()
                    playControl = paused
                    //sender.setImage(UIImage(named: "play"), for: .normal)
                }
                else if(toolBarPauseButton.currentImage?.isEqual(UIImage(named: "play")))!
                {
                    AudioPlayer.play()
                    playControl = playing
                    //sender.setImage(UIImage(named: "pause"), for: .normal)
